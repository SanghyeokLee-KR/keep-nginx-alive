...
> **핵심 제약 조건:** 규칙상 공격자(교수)를 **차단·권한제한·명령제한할 수 없다.** (포트폴리오의 방어 전략이 '격리'와 '우회'에 집중된 이유다.)
...
> **이 프로젝트의 정의** — 본 프로젝트는 root 권한 공격자를 완전히 차단하려는 게 아니라, 제한된 CTF 환경에서 nginx 서비스의 복구 시간 단축과 장애 격리를 높이기 위한 가용성 실험이다. 핵심은 '무적 서버'가 아니라 **장애 감지 → 복구 시도 → 실패 시 트래픽 격리 → 정상 노드로 우회**하는 흐름이다.

> **범위·윤리 고지** — 권한이 부여된 교육용 CTF 경기 박스 안에서 우리 팀 nginx 가용성을 방어한 기록이다. 모든 자동복구·모니터링 기법은 자기 서비스 가용성 유지가 목적이며, 공개 저장소이므로 공인 IP·webhook URL·팀 외 개인정보는 마스킹(`X.X.X.X`, `<REDACTED_WEBHOOK_URL>`)했다.
...
> 본 저장소의 코드·다이어그램·문서는 이상혁이 작성했다. 세 명 모두 경기 기간 새벽 Discord 불침번을 교대로 섰다.
...
`chattr +i`는 `index.html` · `nginx.conf` · nginx 바이너리에 immutable을 건다. 이게 root를 막는 건 아니다(root는 `chattr -i` 한 줄로 푼다). 비root·실수·단순 변조를 막고, 변조에 한 단계 비용을 부과해 감사 로그에 흔적을 남기는 용도다.

watchdog 등을 `nginx-wd-a`·`net-status-mon` 같은 평범한 서비스명으로 둔 건 실무 보안 기법이 아니다. CTF 환경에서 공격자의 단순 탐색 비용을 늘리는 보조 수단일 뿐이고, `systemctl list-units` 로는 그대로 보인다. 보안 경계로 의존하지 않는다.
...
- root 공격자는 로컬 방어를 우회할 수 있다. `chattr -i`·`systemctl stop/disable/mask`·`crontab -r`·`pkill`을 원자적 스크립트로 한 번에 실행하면 상호 복구 메시도 무너진다. 로컬 계층은 최종 방어선이 아니라 복구 시간 단축·격리 트리거일 뿐이다.
- 인스턴스 내부 복구는 실무 HA의 최종 해법이 아니다. "오염된 인스턴스를 끝까지 살리는 것"보다 폐기하고 교체하는 게 옳다.
- `#1` 내부 로그는 공격자가 변조·삭제할 수 있다. auditd·`cmds.json` 모두 공격받는 박스 위에 있어 신뢰할 수 없다. 중앙 로그 저장소가 필요하다.
- watchdog 다중화는 복구성을 높이지만 운영 복잡도를 키운다. 16개 스크립트에 다수 서비스라 유지보수·장애 분석 비용이 늘고, 같은 일(재기동)을 여러 계층이 중복 수행한다.
- 헬스체크 플래핑 가능성. `healthz-checker`에 디바운스가 없어 일시적 `curl` 타임아웃에도 `#1`이 격리될 수 있다. 연속 N회 실패 임계치로 완화할 수 있다(NLB 헬스체크 임계치가 일부 완충).
- 단일 서비스 포트(`:80`) 집중. 규칙상 가용성 공격은 제외됐지만, 실운영이라면 rate limiting·WAF 같은 트래픽 레벨 방어가 없는 게 한계다.
- 관측 산출물을 웹루트에 노출한 건(데모 편의) 실무에선 인증 뒤로 가야 한다.
...
- **인스턴스 교체형 복구** — 인스턴스 내부에서 끝까지 복구하기보다, 오염된 인스턴스를 폐기하고 Auto Scaling Group + Launch Template으로 새 인스턴스로 교체한다. 헬스체크 실패 → 자동 교체.
- **Multi-AZ** — `#1`/`#2`/`#3`를 여러 가용영역에 분산해 AZ 장애에도 견딘다.
- **중앙 로깅 · 원격 관리** — CloudWatch Logs(+ 원격 syslog)로 로그를 박스 밖으로 보내고, SSM으로 접근·명령 감사를 표준화한다.
